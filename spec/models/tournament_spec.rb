...
describe Tournament do
  ...
  describe "generate matches" do
    ...
    it "won't suggest a match between 2 teams if they have already played against eachother" do
      match1 = FactoryGirl.create(:match, home_team_id: tournament_team2.id, away_team_id: tournament_team.id, tournament_id: tournament1.id)
      match2 = FactoryGirl.create(:match, home_team_id: tournament_team4.id, away_team_id: tournament_team3.id, tournament_id: tournament1.id)
      match3 = FactoryGirl.create(:match, home_team_id: tournament_team6.id, away_team_id: tournament_team5.id, tournament_id: tournament1.id)

      @matches = tournament1.schedule
      expect(@matches[0]["match1"]["home"]).to eq(team6.id)
      expect(@matches[0]["match1"]["away"]).to eq(team4.id)
      expect(@matches[0]["match2"]["home"]).to eq(team5.id)
      expect(@matches[0]["match2"]["away"]).to eq(team2.id)
      expect(@matches[0]["match3"]["home"]).to eq(team3.id)
      expect(@matches[0]["match3"]["away"]).to eq(team.id)
    end
